...
class OrderSeeder extends Seeder
{
    ...
    {
        // Load data once to avoid repeated queries
        $tables = Table::all();
        $menuItems = MenuItem::with('modifierGroups.modifierItems')->get();

        // Create 20 orders with various statuses
        $statuses = ['pending', 'confirmed', 'preparing', 'served', 'completed', 'cancelled'];
        ...
        for ($i = 1; $i <= 20; $i++) {
            try {
                ...
                $order = Order::create([
                    ...
                    'completed_at' => $status === 'completed' ? $createdAt->copy()->addHours(1) : null,
                ...
                Payment::create([
                    ...
                    'paid_at' => $paymentStatus === 'paid' ? $createdAt->copy()->addMinutes(rand(5, 30)) : null,
                    'created_at' => $createdAt,
                    'updated_at' => $createdAt,
                ]);
                ...
                // Clear PDO to avoid prepared statement conflicts
                DB::connection()->getPdo()->setAttribute(\PDO::ATTR_EMULATE_PREPARES, true);
            } catch (\Exception $e) {
                $this->command->warn("Skipped order {$i}: " . $e->getMessage());
                continue;
            }
        }

        $this->command->info("Created {$successCount} orders with items, modifiers, and payments");
    }
}
